...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        //search bar
        this.searchBar = rightTabRowHelper.addChild(new EditBox(this.font, 0, 0, 150, 20, Component.translatable("backpack.config.searchbar.hint")), 2);
        searchBar.setHint(Component.translatable("backpack.config.searchbar.hint"));
        searchBar.setResponder(this::pushList);

        //3 buttons in right tab
        ButtonReset_Cancel = rightTabRowHelper.addChild(
                Button.builder(
                                Component.translatable("backpack.config.reset"),
                                button -> {
                                    //This is cancel mode
                                    if (isChanged) {
                                        this.undoConfig();
                        ...
                        .width(75)
                        .tooltip(Tooltip.create(Component.translatable("backpack.config.reset.tooltip")))
                        .build()
        );

        ButtonUndo = rightTabRowHelper.addChild(
                Button.builder(
                                Component.translatable("backpack.config.undo"),
                                button -> this.undoConfig())
                        .width(75)
                        .tooltip(Tooltip.create(Component.translatable("backpack.config.undo.tooltip")))
                        .build()
        );
    ...
    public void tick() {
        ...
        if (ButtonUndo != null && ButtonDone_Save != null && ButtonReset_Cancel != null && isChanged) {
            this.ButtonDone_Save.setMessage(Component.translatable("backpack.config.save").withStyle(ChatFormatting.GREEN));
            this.ButtonDone_Save.setTooltip(Tooltip.create(Component.translatable("backpack.config.save.tooltip")));

            this.ButtonReset_Cancel.setMessage(Component.translatable("backpack.config.cancel").withStyle(ChatFormatting.RED));
            this.ButtonReset_Cancel.setTooltip(Tooltip.create(Component.translatable("backpack.config.cancel.tooltip")));

            this.ButtonUndo.active = true;
        } else {
            this.ButtonReset_Cancel.setMessage(Component.translatable("backpack.config.reset"));
            this.ButtonReset_Cancel.setTooltip(Tooltip.create(Component.translatable("backpack.config.reset.tooltip")));

            this.ButtonDone_Save.setMessage(Component.translatable("backpack.config.exit"));
            this.ButtonDone_Save.setTooltip(Tooltip.create(Component.translatable("backpack.config.exit.tooltip")));
